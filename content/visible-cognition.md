---
title: "Visible Cognition: When the process becomes the artifact"
date: 2025-01-10
tags:
...
1. **Madhyamaka Tetralemma**: Nāgārjuna's four-fold negation examines whether X is true, false, both, or neither—then negates all four to demonstrate emptiness of inherent existence
1. **Neo-Confucian Investigation of Things** (*gewu*): Systematic observation to discern *li* (pattern/principle) until breakthrough (*huoran guantong*)—sudden comprehensive understanding
1. **Two-Eyed Seeing** (*Etuaptmumk*): Mi'kmaw framework for "learning to see from one eye with the strengths of Indigenous ways of knowing, and from the other eye with the strengths of Western ways of knowing, and to use both together"
...
1. **Clinical Reasoning**: Dual process model integrating fast intuitive pattern recognition with slow analytical hypothesis testing
1. **Legal Analysis (IRAC)**: Issue, Rule, Application, Conclusion—the fundamental structure requiring explicit connection between rule and fact
1. **Talmudic Hermeneutics**: Rabbi Ishmael's 13 rules including *kal va-ḥomer* (a fortiori reasoning) and *gezerah shavah* (inference from similar phrases)
...
1. **Cut-Up Technique**: William Burroughs' method of physically cutting and rearranging text to reveal "implicit content" impossible for single conscious mind
1. **Aleatoric Composition**: John Cage's use of I Ching hexagrams to determine musical parameters, surrendering intentional control
1. **Surrealist Techniques**: Automatic writing and exquisite corpse—bypassing rational control to access unconscious
...
1. **Breadth-first exploration**: Most systems use depth-first traversal; users often want "high-level themes first, then selective detail"
1. **Networked reasoning visualization**: Human reasoning involves complex networks of interconnected concepts; current interfaces are purely sequential
1. **Skip/summarize mid-stream**: No current system allows jumping to conclusion mid-generation when user "got the answer"
1. **Collaborative reasoning**: Current tools are solo experiences; team workflows remain unsupported
1. **Reasoning history/provenance**: No robust tracking of how reasoning influenced outputs over conversation
1. **User-personalized reasoning styles**: No systems learn individual engagement preferences
1. **Cross-modal visualization**: Visual chain-of-thought for multimodal reasoning emerging in research but not production
1. **Uncertainty quantification**: Per-claim confidence indicators within reasoning
1. **Alternative path exploration**: Showing reasoning branches AI considered but didn't take
